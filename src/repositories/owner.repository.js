...
async function getOwner(id) {
  ...
  try {
    const res = await conn.query("SELECT * FROM owners WHERE id = $1", [id]);
...
async function updateOwner(owner) {
  ...
  try {
    ...
    const values = [owner.name, owner.phone, owner.id];
...
async function deleteOwner(id) {
  ...
  try {
    const res = await conn.query("DELETE FROM owners WHERE id = $1", [id]);
