...
async function getAnimal(id) {
  ...
  try {
    const res = await conn.query("SELECT * FROM animals WHERE id = $1", [id]);
...
async function updateAnimal(animal) {
  ...
  try {
    ...
    const values = [animal.name, animal.type, animal.owner_id, animal.id];
...
async function deleteAnimal(id) {
  ...
  try {
    const res = await conn.query("DELETE FROM animals WHERE id = $1", [id]);
...
async function getAnimalByOwner(owner_id) {
  const conn = await connect();
